...
int iap_https_init(iap_https_config_t *config)
{
    ...
    http_metadata_request.response_buffer_len = 2048;
    ...
    xTaskCreate(&iap_https_task, "fwup_wifi_task", LARGEST_STACK, NULL, LOW_PRIORITY, ota_task_handle);
...
static void iap_https_prepare_timer()
{
    ...
    if (fwupdater_config->polling_interval_s > 0) {
        ...
        xTimerChangePeriod(check_for_updates_timer, timerPeriod, pdMS_TO_TICKS(30000));
        
        if (pdFAIL == xTimerReset(check_for_updates_timer, pdMS_TO_TICKS(30000))) {
    ...
    if (check_for_updates_timer) {
        ...
        xTimerDelete(check_for_updates_timer, pdMS_TO_TICKS(30000));
