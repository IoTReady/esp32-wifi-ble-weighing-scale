...
char OTA_IP[100] = "10.10.20.214";
char OTA_PORT[100] = "8000";
...
{
    ESP_LOGI(TAG, "Server IP: %s Server Port:%s", OTA_IP, OTA_PORT);
    ...
    sock_info.sin_addr.s_addr = inet_addr((char *) OTA_IP);
    sock_info.sin_port = htons(atoi((char *) OTA_PORT));
...
void local_ota_task(void *pvParameter)
{
    ...
    */
    ...
    int get_len = asprintf(&http_request, GET_FORMAT, OTA_FILENAME, OTA_IP, OTA_PORT);
    if (get_len < 0) {
